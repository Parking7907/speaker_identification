...
Train = glob("/home/data/jinyoung/classification/output_voxceleb_VAD/train/*/*")
Test = glob("/home/data/jinyoung/classification/output_voxceleb_VAD/test/*/*")
#output = "/home/jinyoung/speaker_identification/voxceleb/"
# os.makedirs(new_file_out, exist_ok = True)
f = open("./voxceleb_train.txt", 'w')
f2 = open("./voxceleb_all.txt", 'w')
f3 = open("./voxceleb_test.txt", 'w')
#print(Test)
for name in Train:
    vid_n = os.path.basename(name)
    print(name.split('output_voxceleb_VAD/')[1])
    data = name.split('output_voxceleb_VAD/')[1]
    f.write(data+'\n')
    f2.write(data+'\n')
